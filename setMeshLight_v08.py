...
def createWindow():
    ...
    cmds.rowLayout(nc=4) 
    cmds.checkBox('CPU_Process_Limit_Switch',l='Max Process',v=1)   
    cmds.intSliderGrp('CPU_Process_Limit_Percent',w=200,field=True,enable=False,min=1,max=100)
    cmds.text('CPU_Process_Limit_Max',w=35,l='0',enable=False)
    cmds.text('CPU_Process_Limit_Total_Count',l=str(multiprocessing.cpu_count()),w=35)
    ...
    cmds.checkBox('CPU_Process_Limit_Switch',e=True,cc=lambda *args:setCpuLimited())
    cmds.intSliderGrp('CPU_Process_Limit_Percent',e=True,cc=lambda *args:setCpuLimited())
    
def setCpuLimited():
    if cmds.checkBox('CPU_Process_Limit_Switch',q=True,v=True) is True:
        cmds.control('CPU_Process_Limit_Percent',e=True,enable=False)
        cmds.control('CPU_Process_Limit_Max',e=True,enable=False)
        cmds.setAttr('defaultArnoldRenderOptions.threads_autodetect',1)
    else:
        cmds.control('CPU_Process_Limit_Percent',e=True,enable=True)
        cmds.setAttr('defaultArnoldRenderOptions.threads_autodetect',0)
        
        percent = cmds.intSliderGrp('CPU_Process_Limit_Percent',q=True,v=True)
        count = int(cmds.text('CPU_Process_Limit_Total_Count',q=True,l=True))
        cmds.text('CPU_Process_Limit_Max',e=True,l=int(math.floor(count*(percent/100.0))))
        cmds.setAttr('defaultArnoldRenderOptions.threads',int(cmds.text('CPU_Process_Limit_Max',q=True,l=True)))
